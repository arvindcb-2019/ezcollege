<html>
	...
	<body>
		...
		?>
		<center>
		<h3>Modify Marks for Students</h3>
		<div class="col-md-6">
		<form action="modifymarkpost" method="post">
			<table border=1>
				<tr id="first">
					<td>Group</td>
		...
		?>
			...
			<form action="modifymarkprocess" method="post">
		<center>
				<tr>
					...
					<?php
						$testname = $con->query("SELECT DISTINCT(testname) AS tname FROM marks WHERE groupname='$group' AND slot='$slot'");
						while($testrow = $testname->fetch_assoc()){
					?>
						<option value="<?php echo $testrow['tname'] ?>"><?php echo $testrow['tname'] ?></option>
						<?php } ?>
					</select>
					</td>
				</tr>
		...
		<?php
				...
				<script>
					swal('Unable to modify marks', 'You do not have any <?php echo $slot ?> slot classes in <?php echo $group ?>, try again by changing the group or slot.', 'error').then(() => {
